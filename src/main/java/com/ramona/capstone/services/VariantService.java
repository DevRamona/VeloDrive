...
public class VariantService {
  ...
  public List<VariantDto> getVariantsByProduct(Long productId) {
    ...
    return variantRepository.findByProductId(productId).stream().map(variantMapper::toDto).toList();
  }
            ...
            .orElseThrow(
                () ->
                    new ResourceNotFoundException(
                        "Cannot add variant. Product not found with ID:" + productId));
  ...
  public VariantDto updateStock(String sku, UpdateStockDto stockUpdate) {
    Variant variant =
        variantRepository
            .findBySku(sku)
            .orElseThrow(
                () -> new ResourceNotFoundException("Variant with SKU" + sku + "not found"));
